...
│   │   ├── optimal_mixed_precision.py        the 6/32 surgical model (Sec 3.4, App C)
│   │   ├── multi_seed_significance.py        5-seed McNemar validation (Sec 3.4, App A)
│   │   ├── math_difficulty_stratification.py MATH null analysis (Sec 3.5, App E)
│   │   ├── additional_benchmarks.py          HellaSwag + MMLU contrast (Sec 3.5)
│   │   └── patch_count_ablation.py           2/4/6/8-layer ablation
│   │
│   ├── mechanistic/                       <- Hessian-vs-functional analysis (Sec 3.6, 4.3)
│   │   ├── layer_sensitivity_profiler.py     64-layer recovery sweep (Fig 2, App B)
│   │   ├── sensitivity_ranking_comparison.py Hessian trace + rho = -0.506 (Fig 1)
│   │   ├── hessian_vs_restoration.py         HESS-6 vs REST-6 head-to-head (Sec 4.3, Tab 3)
│   │   ├── mistral_hessian_replication.py    Mistral replication (rho = -0.574)
│   │   ├── cross_task_interference.py        5x3 patch-by-task matrix (Fig 3)
│   │   ├── bidirectional_flip_analysis.py    forward/reverse flip breakdown (App F)
│   │   └── baseline_comparison.py            FP16/NF4/GPTQ baselines (Tab 1)
│   │
│   ├── multi_model/                       <- cross-architecture probe (Sec 4.1, App H)
...
| File | Paper element |
|---|---|
| `llama/quantization_damage.json` | Section 2.2, baseline NF4 damage |
| `llama/baseline_comparison.json` | Table 1 (FP16 / NF4 / GPTQ across 6 benchmarks) |
| `llama/surgical_report.json` | Section 3.1, surgical transplant core results |
...
| `llama/math_difficulty_stratification_report.json` | Section 3.5, Appendix E |
| `llama/additional_benchmarks_report.json` | Section 3.5 (HellaSwag + MMLU contrast) |
...
| `llama/sensitivity_ranking_comparison.json` | Figure 1, Section 3.6 (ρ = −0.506) |
| `llama/bidirectional_flip_report.json` | Appendix F (forward/reverse flips) |
| `llama/hessian_vs_restoration.json` | **Table 3, Section 4.3 (HESS-6 vs REST-6, p = 0.001)** |
| `mistral/mistral_replication_report.json` | Section 4.1 (cross-architecture) |
| `mistral/mistral_hessian_replication.json` | Section 4.1, Appendix I (Mistral ρ = −0.574) |
...
# 4. core probe (LLaMA-3.1-8B-Instruct) — Section 3
...
# 5. Hessian inversion analysis — Section 3.6, 4.3
...
# 6. cross-architecture replication — Section 4.1
